...
namespace Burrows.NHib.Saga
{
    public class NHibernateSagaRepository<TSaga> :
        ISagaRepository<TSaga>
        where TSaga : class, ISaga
    {
        private static readonly ILog _logger = Logger.Get(typeof(NHibernateSagaRepository<TSaga>).ToFriendlyName());
            ...
            where TMessage : class
        {
            ...
            using (ITransaction transaction = session.BeginTransaction())
            {
                ...
                if (instance == null)
                {
                        ...
                        yield return x =>
                            {
                                if (_logger.IsDebugEnabled)
                                    _logger.DebugFormat("SAGA: {0} Creating New {1} for {2}",
                    ...
                    {
                        if (_logger.IsDebugEnabled)
                            _logger.DebugFormat("SAGA: {0} Ignoring Missing {1} for {2}", typeof(TSaga).ToFriendlyName(),
                                sagaId,
                                typeof(TMessage).ToFriendlyName());
                    }
                }
                else
                {
                    if (policy.CanUseExistingInstance(context))
                    {
                        yield return x =>
                            {
                                if (_logger.IsDebugEnabled)
                                    _logger.DebugFormat("SAGA: {0} Using Existing {1} for {2}",
                                        typeof(TSaga).ToFriendlyName(), sagaId,
                                        typeof(TMessage).ToFriendlyName());
                    ...
                    {
                        if (_logger.IsDebugEnabled)
                            _logger.DebugFormat("SAGA: {0} Ignoring Existing {1} for {2}", typeof(TSaga).ToFriendlyName(),
                                sagaId,
                                typeof(TMessage).ToFriendlyName());
                    }
                }
